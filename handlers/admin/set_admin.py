...
def set_del_admin_for_groups(message):
    """
    Команда назначают права администратора «Группы» у пользователей
    :param message:
    :return:
    """
    ...
    try:
        ...
        bot.send_message(message.chat.id, 'Неправильный формат команды. Формат: /set_admin groupname username')
        return
    ...
    db.update_user_in_group_admin(telegram_group_id, telegram_user_id, True)
    bot.send_message(message.chat.id, f'Пользователь {username} теперь админ группы {group_name}')
